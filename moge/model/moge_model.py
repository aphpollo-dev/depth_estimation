...
class MoGeModel(nn.Module):
    ...
    def forward(self, image: torch.Tensor, mixed_precision: bool = False) -> Dict[str, torch.Tensor]:
        ...
        with torch.autocast(device_type='cpu', dtype=torch.bfloat16, enabled=mixed_precision, cache_enabled=True):
            print("start")
    ...
    def infer(
        ...
        if expected_area != area:
            expected_width, expected_height = int(original_width * (expected_area / area)), int(original_height * (expected_area / area))
